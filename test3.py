...
def capture_image(camera, camera_matrix, dist_coeffs, detector):
    """ถ่ายและบันทึกภาพ"""
    camera.release()
    camera = setup_camera()
    ret, frame = camera.read()
    if ret:
        ...
        if len(tags) >= 2:
            ...
            original_path = f"images/original/original_image_{timestamp}.jpg"
            cv2.imwrite(original_path, undistorted)
            print(f"บันทึกภาพต้นฉบับที่: {original_path}")
            
            # บันทึกภาพครอปพร้อมเส้น
            cropped_path = f"images/cropped/cropped_image_{timestamp}.jpg"
                    ...
                    try:
                        if ser.is_open and ser.in_waiting > 0:
                            ...
                            if command == "CAMERA_CAPTURE":
                                camera = capture_image(camera, camera_matrix, dist_coeffs, detector)
                                ser.write(b'CAPTURE_DONE\n')
                                print("ถ่ายภาพเสร็จแล้ว ส่งการยืนยันกลับไป")
