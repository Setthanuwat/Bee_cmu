...
f44ffkgehffkregehgrrhreheeeghrhr
@author: avsthiago
"""
...
def find_circles(im_name, img, mask, cnt, output_size=64):
    ...
    try:
      ...
      #  cv2.waitKey(1000)
        ...
        blurred = cv2.GaussianBlur(gray, (5, 5), 0)  # ลดขนาด kernel และปรับ sigma
       ...
       # cv2.waitKey(1000)
        ...
        circles = cv2.HoughCircles(
            blurred,
            cv2.HOUGH_GRADIENT,
            dp=4.7 ,  # ลดจาก 10 เป็น 1n 
            minDist=12,  # เพิ่มระยะห่างขั้นต่ำระหว่างวงกลม
            param1=80,  # ค่า gradient ขั้นต่ำ
            param2=10,  # threshold ของ accumulatorv
            minRadius=6,  # เพิ่มขนาดขั้นต่ำ
            maxRadius=12  # เพิ่มขนาดสูงสุด
        )

        # If circles are detecnted, process them
        if circles is not None:
           ...
           # cv2.waitKey(0)  # รอให้กดปุ่มใดๆ ก่อนปิด
